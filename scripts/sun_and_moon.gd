...
##on earth, dawn and dusk are exactly 1/10th as long as day
##night is 144/180 (80%, 8/10) the length of day
const DAWN_LENGTH : float = (5.0/10.0)
const DAY_LENGTH : float = 5.0
const DUSK_LENGTH : float= (5.0/10.0)
const NIGHT_LENGTH : float = 5.0 * (8.0/10.0)
...
var time : float
@export var editorTime : float :
	set(value):
		time = value
		if Engine.is_editor_hint():
			var ranInEditor = runInEditor
			runInEditor = true
			_physics_process(0)
			runInEditor = ranInEditor
	get:
		return time
@export var runInEditor : bool = false
@export var timeScale : float = 60.0
...
func _physics_process(delta: float) -> void:
	...
	if Engine.is_editor_hint() and !runInEditor:
		return
	
	if multiplayer.is_server() or Engine.is_editor_hint():
